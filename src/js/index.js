...
function $(selector) {
  const Href =window.location.pathname.split(".html").join("").split("/")
  const currentHref =Href[Href.length-1]
  return new Dom(selector,null, {currentHref})
}
class Dom {
  ...
  Components = [
      ...
      const html= `<div id="hamburger-open"><span class="spans" id="spanOne" data-span="span"></span><span class="spans" id="spanTwo" data-span="span"></span><span class="spans" id="spanThree" data-span="span"></span></div>
          <div class="modal-menu">
                  ...
                  <div class="dropdownItem offItem service-inner" data-id="2">
                      ...
                      <a href="/modules/quality-telem.html" class="menuitem">Сервис «Расчет транспортной работы по данным навигационных треков»</a>
                      <a href="/modules/quality-telem.html" class="menuitem">Сервис «Мониторинг пассажиропотока на маршрутах ГПТ по данным от датчиков» в соответствии с ГОСТ 54-723-2019</a>
    ...
    function Navigation() {
      var tag = document.createElement("div");
      tag.classList.add("frame")
      const html =`
      <div class="frame__title-wrap">
        <div class="navig work-type">
          <div class="final-drop">
          <a class="frame__tagline">Новости</a>
          <div class="dropdownItem offItem" data-id="2">
          </div>
          </div>
          <div class="final-drop">
          <a class="frame__tagline">Клиентам</a>
            <div class="dropdownItem offItem" data-id="2">
            </div>
          </div>
          <div class="final-drop">
            <a class="frame__tagline dropdown">Сервисы</a>
            <div class="dropdownItem offItem" data-id="2">
              <a href="/" class="menuisem">Сервис «Ведение паспортов маршрутов в соответствии с ФЗ 220 с использованием встроенной ГИС»</a>
              <a href="/src/pages/services/telem.html" class="menuisem">Сервис «Мониторинг качества телематических данных и работоспособности оборудования»</a>
              <a href="/index4.html" class="menuisem">Сервис «Формирование интервальных расписаний»</a>
              <a href="/modules/gis.html" class="menuisem">Сервис «Формирование объявленных расписаний»</a>
              <a href="/index6.html" class="menuisem">Сервис «Расчет транспортной работы по данным навигационных треков»</a>
              <a href="/src/pages/services/monitor.html" class="menuisem">Сервис «Мониторинг пассажиропотока на маршрутах ГПТ по данным от датчиков» в соответствии с ГОСТ 54-723-2019								</a>
              <a href="/src/pages/services/analiz.html" class="menuitsem">Сервис «Расчет и анализ маршрутного модельного пассажиропотока»								</a>
              <a href="/index9.html" class="menuisem">Сервис «Расчет и анализ пассажиропотока по данным валидаций АСОП и навигационным трекам» соответствии с ГОСТ 54-723-2019								</a>
              <a href="/index10.html" class="menuisem">Сервис «Анализ модельного пассажиропотока на маршрутной сети»</a>
            </div>
          </div>
    
        </div>
        <div class="navig edu-type off">
          <a class="frame__tagline">Новости</a>
          <a class="frame__tagline">ВУЗам</a>
          <a class="frame__tagline dropdown">ЗУВ</a>
          <div class="dropdownItem offItem" data-id="3">
            <a href="/index11.html" class="menuitem">ЗУВ: «Мониторинг поступления телематических данных от оборудования»								</a>
            <a href="/index12.html" class="menuitem">ЗУВ: «Сбор и ввод данных табличного обследования пассажиропотоков»								</a>
            <a href="/index13.html" class="menuitem">ЗУВ: «Сбор и ввод данных силуэтного метода обслудования пассажиропотоков»								</a>
        ...
        <div class="btn-custom-tab">
          <a class="tab-link active-tab" data-id="1">Интеграторам </br>/ Перевозчикам</a>
          <a class="tab-link" data-id="2">ВУЗам</a>
        </div>
      </div>
      `
      tag.innerHTML=html
    ...
    function HeaderText(props){
      const {currentHref, headerTitle={
        first:"Ведение паспортов маршрутов в соответствии с ФЗ 220 с использованием встроенной ГИС", 
        second:"Получение навыков по заполнению справочной информации по подвижному составу, телематическому оборудованию, маршрутной сети."
      }}=props
     
      var tag = document.createElement("div");
      tag.classList.add("header-title")
      const html =`
      <h1 class='work-type work-type-title'>
        ${headerTitle.first}
      </h1>
      <h1 class='edu-type off'>
      ${headerTitle.second}
      </h1>
      <p class="chenge-module module-text" id=0></p>
      <p class="chenge-module off" id=1></p>
      `
      tag.innerHTML=html
      return tag
      },
    function MadiIcon() {
      var tag = document.createElement("div");
      tag.classList.add("frame__demos")
      const html =`
      <div class="madi">
        <p>Технологии, разработанные при участии кафедры «Транспортная телематика» МАДИ</p>
        <div class="logo">
          <img src="src/img/madi.png" alt="">
        </div>
      </div>
      `
      tag.innerHTML=html
    ...
    function Slider() {
      ...
      <nav style="background-color: transparent;">
            <a href="#" class="main-nav next" data-nav="next" data-id=0>Next <span></span></a>
              </nav>
  `
    tag.innerHTML=html
    return tag
  ...
  defineProps =(el, props)=> {
    console.log(props.headerTitle, "name")
    const {currentHref}=props
    let newProps= {...props}
    tag=currentHref || "index"


    if (currentHref=="telem") {
      // tag="pass"
      newProps={headerTitle:{
        first:"Мониторинг пассажиропотока по данным от датчиков", 
        second:"Получение навыков по заполнению справочной информации по подвижному составу, телематическому оборудованию, маршрутной сети."
      }}
    }
    if(currentHref=="monitor") {
      newProps={headerTitle:{
        first:"Мониторинг качества телематических данных и работоспособности оборудования", 
        second:"Получение навыков по заполнению справочной информации по подвижному составу, телематическому оборудованию, маршрутной сети."
      }}
    }
    if(currentHref=="analiz") {
      newProps={headerTitle:{
        first:"Расчет и анализ маршрутного модельного пассажиропотока", 
        second:"Получение навыков по заполнению справочной информации по подвижному составу, телематическому оборудованию, маршрутной сети."
      }}
    }
    return newProps
  ...
  async render(type) {
    ...
    if (type) {
      this.Components.forEach((el, idx)=> {
        if (type==el.name) {
          props= this.defineProps(el, this.props)
          this.sel.appendChild(el(props))
          this.setState((prev)=> {
            const {count}=prev
...
const workList = {
  ...
  telem: [
    ...
    [
      '</br>«Модуль мониторинга качества телематических данных и работоспособности оборудования »',
      `<a href="index3.html" class="btn-custom block ">Подробнее...</a>`
    ],
    [
      '</br>«Модуль обработки первичных данных от телематического оборудования и датчиков пассажиропотока»',
      `<a href="index3.html" class="btn-custom block ">Подробнее...</a>`
    ],
    [
      '</br>«Модуль анализа пассажиропотока в формате табличного метода обследования»',
      `<a href="index3.html" class="btn-custom block ">Подробнее...</a>`
    ]
  ],
  monitor: [
  ...
  analiz: [
    ...
    [
      '</br>«Модуль ведения справочников подвижного состава и датчиков пассажиропотока»',
      `<a href="index3.html" class="btn-custom block ">Подробнее...</a>`
    ],
    [
      '</br>«Модуль мониторинга качества телематических данных и работоспособности оборудования»',
    ...
    [
      '</br>«Модуль расчета и анализа модельного пассажиропотока»',
      `<a href="index3.html" class="btn-custom block ">Подробнее...</a>`
    ]
  ],
...
tabLink.forEach((elem)=> {
    elem.addEventListener("click", (e)=> {
        ...
        if (elem.dataset.id == 1) {
          console.log(workList[tag][0], "workList.index[0]")
          moduleText.innerHTML= workList[tag][0]
